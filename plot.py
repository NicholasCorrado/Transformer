...
def visualize_all_curves(train_loss, val_loss, train_loss2, val_loss2, name):
  ...
  ax.set_title(name)
  # plot lines
  ax.plot(indices, train_loss, label="Training a", c='blue')
  ax.plot(indices, val_loss, label="Validation a", c='orange')
  ax.plot(indices, train_loss2, label="Training c", c='cyan')
  ax.plot(indices, val_loss2, label="Validation c", c='yellow')
